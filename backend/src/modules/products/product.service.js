...
const createProduct = async (data) => {
  ...
  return prisma.product.create({
    data: {
      ...
      images: data.images || [],
      filterValues: {
        // Connect checked frontend filter value IDs directly through relational join table
        connect: data.filterValueIds
          ? data.filterValueIds.map((id) => ({ id }))
          : [],
      },
    },
...
const getProducts = async ({
  ...
  minPrice,
  maxPrice,
  filterValueIds, // Expecting a string or an array of string filter value UUIDs
}) => {
  const skip = (page - 1) * limit;
  const where = {};
  ...
  // Multi-checkbox filtering handling logic
  if (filterValueIds) {
    const idsArray = Array.isArray(filterValueIds)
      ? filterValueIds
      : [filterValueIds];
    where.filterValues = {
      some: {
...
const getProductBySlug = async (slug) => {
  return prisma.product.findUnique({
    ...
    include: {
      subcategory: true,
      variants: true,
      filterValues: { include: { filterGroup: true } },
    },
  });
};
...
const updateProduct = async (id, data) => {
  ...
  return prisma.product.update({
    ...
    data: {
      ...
      subcategoryId: data.subcategoryId,
      images: data.images || [],
      filterValues: data.filterValueIds
        ? {
            // Disconnect old filter configurations completely and replace with newly updated selection overrides
            set: data.filterValueIds.map((id) => ({ id })),
          }
        : undefined,
    },
  });
  ...
  for (let i = 0; i < parsedRows.length; i++) {
    ...
    try {
      if (
        ...
        throw new Error(
          "Missing mandatory fields (name, price, categoryName, or subcategoryName)",
        );
      }
      ...
      // Resolve category
      const catSlug = slugify(row.categoryName.trim(), {
        lower: true,
        strict: true,
      });
      let category = await prisma.category.findUnique({
      ...
      if (!category) {
        category = await prisma.category.create({
          data: { name: row.categoryName.trim(), slug: catSlug },
        });
      }

      // Resolve subcategory inside that category context
      const subcatSlug = slugify(row.subcategoryName.trim(), {
        lower: true,
        strict: true,
      });
      ...
      summary.skippedCount++;
      summary.errors.push(`Row ${rowNumber}: Failed - ${err.message}`);
    }
  }
