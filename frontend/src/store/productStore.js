...
const frontendProductSchema = z.object({
  ...
  stock: z.preprocess(
    (val) => (val === "" || val === null ? undefined : Number(val)),
    z.number({ required_error: "Stock is required", invalid_type_error: "Stock must be a number" })
...
export const useProductStore = create((set, get) => ({
  ...
  fetchCatalog: async () => {
    ...
    try {
      ...
      set({ globalError: "Could not download system catalog dashboards.", loading: false });
    }
  },
  ...
  createProduct: async (payload) => {
    ...
    try {
      ...
      set((state) => ({ products: [created, ...state.products], submitLoading: false }));
      return { success: true };
    } catch (err) {
...
function handleAxiosValidationError(err, set) {
  ...
  if (Array.isArray(validationDetails)) {
    ...
    set({ globalError: serverMessage || "Action execution failed.", submitLoading: false });
  }
  return { success: false };
}