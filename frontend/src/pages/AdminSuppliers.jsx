...
export default function AdminSuppliers() {
  ...
  // Fetch suppliers whenever the 'page' state changes
  const fetchSuppliers = async () => {
    try {
      setLoading(true);
  ...
  // Crucial: Added 'page' as a dependency so it fetches new data when page changes
  useEffect(() => {
    fetchSuppliers();
  }, [page]);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      const maxRemainingPages = Math.ceil(updatedTotalItems / limit) || 1;
      if (page > maxRemainingPages) {
        setPage(maxRemainingPages);
      } else {
        // Refresh current page data to fill the gap left by deleted row
        fetchSuppliers();
      }
    } catch (err) {
      alert(err.response?.data?.error || "Failed to delete supplier.");
  ...
  // Calculate local index offsets based on current page context
    ...
    <div className="mx-auto max-w-7xl p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-blue-500 transition-colors"
        >
          Add Supplier
        </button>
      </div>
      ...
      {/* Styled Inventory-grade Table Wrapper */}
